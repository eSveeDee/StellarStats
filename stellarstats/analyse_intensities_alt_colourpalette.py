...
def get_ratio_mask(mask, ratios_df, input_filename, output_dir="./", save_output=False, vmax=None):
    ...
    Parameters:
        ...
        vmax (float): Maximum value for the colormap scaling.
    """
    ...
    # Define the original Carto Temps colors
    carto_temps_colors = [
        "#b55288",
        "#f9f9e7",
        "#6daa8d"
    ...
    # Create a continuous colormap from the inverted colors
    carto_temps_cmap = LinearSegmentedColormap.from_list("carto_temps", carto_temps_colors, N=256)

    # Convert the colormap to an array of RGBA values
    cmap_array = carto_temps_cmap(np.linspace(0, 1, 256))

    # Modify the first color (corresponding to `0`) to white
    cmap_array[0] = [1, 1, 1, 1]  # White (R, G, B, A)

    # Create a new colormap with the modified array
    custom_cmap = LinearSegmentedColormap.from_list("custom_carto_temps", cmap_array)

    # Define vmin and vmax explicitly if not provided
    if vmax is None:
        vmax = mask_copy.max()

    # Plot using the custom colormap
    plt.figure(figsize=(10, 6))
    im = plt.imshow(mask_copy, cmap=custom_cmap, interpolation='nearest', vmin=0, vmax=vmax)
    plt.colorbar(im, label='Ratio Values')
